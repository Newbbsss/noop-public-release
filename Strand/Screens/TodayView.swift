...
struct TodayView: View {
    ...
    private var synthesisSection: some View {
        ...
        VStack(alignment: .leading, spacing: NoopMetrics.gap) {
            ...
            // Recovery vitals card removed (8.6.140 parity) — HRV / RHR / respiratory stay in Key Metrics.
        }
    }
    ...
    private func restRing(diameter: CGFloat) -> some View {
        if let s = restScore {
            ...
            // #898: Charge present but Rest absent — say WHY. Fable Rest #28 / #202: if the day
            // already banks sleep minutes (session exists, score pending), never imply "no night".
            if (displayDay?.totalSleepMin ?? 0) > 0 {
                emptyHeroRing(diameter: diameter) { ringRestUnscored() }
            } else {
    ...
    /// Fable #202 / Rest #28: session exists but Rest not resolved yet — parity with Android RingRestUnscored.
    @ViewBuilder
    private func ringRestUnscored() -> some View {
        VStack(spacing: 3) {
            Text("No Rest score yet").font(StrandFont.headline).foregroundStyle(StrandPalette.textTertiary)
                .lineLimit(1).minimumScaleFactor(0.7).fixedSize()
            Text("open Sleep").font(StrandFont.footnote).foregroundStyle(StrandPalette.textSecondary)
    ...
    private func ringNeedsTrackedNight() -> some View {
        VStack(spacing: 3) {
            // Fable Rest #28: parity with Android RingNeedsTrackedNight — never "Calibrating"
            // (that word is Charge's 4-night baseline gate). Rest is scored-sleep only.
            Text("No Rest score").font(StrandFont.headline).foregroundStyle(StrandPalette.textTertiary)
                .lineLimit(1).minimumScaleFactor(0.7).fixedSize()
            Text("needs a night").font(StrandFont.footnote).foregroundStyle(StrandPalette.textSecondary)
                .lineLimit(1).minimumScaleFactor(0.6).fixedSize()
        }
    }
