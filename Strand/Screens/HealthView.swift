        ...
        return VStack(alignment: .leading, spacing: NoopMetrics.space4) {
            ...
            Button { fitnessSheet = .trend } label: {
                HStack(alignment: .center, spacing: NoopMetrics.space5) {
                    // Soft frost number (8.6.139/140 parity) — no liquid vessel on Fitness Age.
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Fitness Age").strandOverline()
                        HStack(alignment: .firstTextBaseline, spacing: 4) {
                            CountUpNumber(value: Double(shown), font: StrandFont.rounded(36))
                    ...
                    .frostedCardSurface(tint: StrandPalette.chargeColor, cornerRadius: 18, washStrength: 0.9)
                    Spacer(minLength: 0)
                    if let vo2 = vo2max {
