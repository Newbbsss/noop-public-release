...
struct SmartAlarmView: View {
    ...
    private var windowHero: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                // Android Sleep|Alarm Bedtime|Wake dual — same glance shape on iPhone (honest: no phone exact-alarm).
                HStack(alignment: .firstTextBaseline, spacing: 14) {
                    ...
                    heroTime(label: "Wake",
                             time: timeLabel(behavior.smartAlarmEnabled ? behavior.smartAlarmMinutes : wakeMinutes),
                             tint: StrandPalette.restBright)
                    Spacer(minLength: 0)
                }
                Text(windDownOn
                     ? "Bedtime nudge \(WindDownNudge.sleepNeedMinutes / 60)h \(WindDownNudge.leadMinutes)m before wake. Strap buzz arms below."
                     : "Set Bedtime (wind-down) and Wake (strap buzz) below. Phone smart-wake stays Android-only.")
        ...
        .accessibilityLabel("Bedtime \(windDownOn ? timeLabel(WindDownNudge.nudgeMinuteOfDay()) : "off"), Wake \(timeLabel(behavior.smartAlarmEnabled ? behavior.smartAlarmMinutes : wakeMinutes))")
    }
    ...
    // Android Alarm Band + Cues switch-list parity: Band buzz + Wind-down at a glance.
    // Turn-back / Wake-rested stay Android-only (phone exact-alarm primitives).
    private var cuesCard: some View {
        StrandCard(padding: 20) {
            VStack(alignment: .leading, spacing: 14) {
                Text("Cues").strandOverline()
                Text("Band + Wind-down")
                    .font(StrandFont.title2)
                    .foregroundStyle(StrandPalette.textPrimary)
                cueRow(
                    title: "Band buzz",
                    detail: "Strap firmware wake buzz",
                ...
                cueRow(
                    title: "Wind-down",
                    detail: "Evening bedtime nudge",
                    isOn: $windDownOn,
                    a11y: "Wind-down"
                )
                .onChangeCompat(of: windDownOn) { on in WindDownNudge.setEnabled(on) }
                Text("Turn-back and Wake-when-rested need Android phone exact-alarm — not available on this shell.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private func cueRow(title: LocalizedStringKey, detail: LocalizedStringKey,
                        isOn: Binding<Bool>, a11y: LocalizedStringKey) -> some View {
