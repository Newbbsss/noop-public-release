    ...
    private var appearanceCard: some View {
        SettingsSection(
            ...
            VStack(spacing: 0) {
                ...
                .tint(StrandPalette.accent)
                Text("How see-through the cards (Heart Rate, Key Metrics, …) are. Left = solid, right = clear.")
                    .font(StrandFont.caption)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
                    .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private var aboutCard: some View {
        SettingsSection(
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                // Invite friends on the same Tailscale tailnet (install links only — no cloud vitals).
                VStack(alignment: .leading, spacing: NoopMetrics.space2) {
                    ShareLink(
                        item: UpdateChecker.friendsTailnetShareText,
                        subject: Text("NOOP on Tailscale"),
                        message: Text("Install NOOP from Gilbert's Tailnet store / AltStore."),
                    ) {
                        Label("Invite friends (Tailnet)", systemImage: "square.and.arrow.up")
                    }
                    .buttonStyle(NoopButtonStyle(.secondary))
                    Text("Friends need Tailscale on your tailnet. Android: store URL. iPhone: AltStore source below.")
                        .font(StrandFont.caption)
                        .foregroundStyle(StrandPalette.textTertiary)
                    Button {
                        #if canImport(UIKit)
                        UIPasteboard.general.string = UpdateChecker.altStoreSourceURL
                        #elseif canImport(AppKit)
                        NSPasteboard.general.clearContents()
                        NSPasteboard.general.setString(UpdateChecker.altStoreSourceURL, forType: .string)
                        #endif
                    } label: {
                        Label("Copy AltStore source URL", systemImage: "doc.on.doc")
                    }
                    .buttonStyle(NoopButtonStyle(.tertiary))
                    Text(UpdateChecker.altStoreSourceURL)
                        .font(StrandFont.caption)
                        .foregroundStyle(StrandPalette.textTertiary)
