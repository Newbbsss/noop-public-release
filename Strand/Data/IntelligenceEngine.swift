...
final class IntelligenceEngine: ObservableObject {
    ...
    static let effortRescoreFlagKey = "intelligence.effortRescore.v323.done"
    ...
    func analyzeRecent(maxDays: Int = 21, force: Bool = true) async {
        ...
        // Fable Rest #9: sleep_performance series backfill — days with DailyMetric sleep but no
        // series point yet this pass (just-synced; import/HC while Charge still calibrating).
        var restDaysCovered = Set(restPoints.map { $0.day })
        var backfillPool: [DailyMetric] = dailies
        let priorComputed = (try? await store.dailyMetrics(deviceId: computedId, from: oldestDay, to: newestDay)) ?? []
        let computedDays = Set(dailies.map { $0.day })
        backfillPool.append(contentsOf: priorComputed.filter { !computedDays.contains($0.day) })
        for source in [deviceId, Repository.appleHealthSource] + Repository.wearableImportSources {
            let rows = (try? await store.dailyMetrics(deviceId: source, from: oldestDay, to: newestDay)) ?? []
            backfillPool.append(contentsOf: rows)
        }
        for pt in Self.backfillSleepPerformanceSeries(
            existingRestDays: restDaysCovered,
            dailyRows: backfillPool
        ) {
            restPoints.append(pt)
            restDaysCovered.insert(pt.day)
        }
    ...
    /// already have a `sleep_performance` day in `existingRestDays`, emit a series point via
    /// `AnalyticsEngine.Rest.composite`. First day wins; never invents sleep.
    nonisolated static func backfillSleepPerformanceSeries(
        existingRestDays: Set<String>,
        dailyRows: [DailyMetric]
    ) -> [MetricPoint] {
        var covered = existingRestDays
        var out: [MetricPoint] = []
        for row in dailyRows {
            guard !covered.contains(row.day),
                  let rest = AnalyticsEngine.Rest.composite(daily: row) else { continue }
            out.append(MetricPoint(day: row.day, key: "sleep_performance", value: rest))
            covered.insert(row.day)
        }
        return out
    }
