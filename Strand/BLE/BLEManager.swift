...
public final class BLEManager: NSObject, ObservableObject {
    ...
    /// engineering. Gated on the same capture toggle; no-op otherwise. Inline-decodes 1244-B IMU (#476).
    ...
    private func connectCore(model: WhoopModel) {
        ...
        selectedModel = model
        // Battery "~X days left" fallback (#713): resolve family via live detect + picker + persisted
        // so a bonded MG is never rated as WHOOP 4 (Android resolveForEstimates parity).
        let estimateModel = WhoopModel.resolveForEstimates(
            selected: model,
            whoop5Detected: model.isWhoop5Family || selectedModel.isWhoop5Family,
            persisted: WhoopModel.persisted)
        state.batteryRatedHours = BatteryEstimator.ratedLifeHours(whoop5Family: estimateModel.isWhoop5Family)
    ...
    /// One-shot user buzz (#921). Twin of Android `WhoopBleClient.buzzStrapOnce` + Ryan whoop_buzz.py:
    /// WHOOP 5/MG fires maverick notify **withoutResponse** first (the path that vibrates), then an
    /// acknowledged retry + RUN_ALARM rev2. WHOOP 4 keeps pattern + RUN_ALARM with response.
    func buzzStrapOnce() {
        if selectedModel.deviceFamily == .whoop5 {
            send(.runHapticsPattern, payload: [2, 3, 0, 0, 0], writeType: .withoutResponse)
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.40) { [weak self] in
                self?.send(.runHapticsPattern, payload: [2, 3, 0, 0, 0], writeType: .withoutResponse)
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.85) { [weak self] in
                self?.send(.runHapticsPattern, payload: [2, 3, 0, 0, 0], writeType: .withResponse)
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.30) { [weak self] in
                self?.send(.runAlarm, payload: AlarmPayload.runAlarmRev2(), writeType: .withResponse)
            }
            log("Buzz: one-shot fired (5/MG Ryan no-resp×2 + acked + runAlarm rev2)")
            return
        }
...
extension BLEManager: @preconcurrency CBCentralManagerDelegate {
    ...
    public func centralManager(_ central: CBCentralManager,
        ...
        cancelScanFallback()
        // Infer family from BLE name when possible so MG never stays labeled WHOOP 4 after reconnect.
        if let inferred = WhoopModel.fromBleName(name) {
            selectedModel = inferred
        }
