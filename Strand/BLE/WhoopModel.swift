...
public enum WhoopModel: String, CaseIterable, Identifiable, Hashable {
    ...
    /// True for 5.0 / MG (shared puffin family + ~12-day pack). Mirrors Android `isWhoop5Family`.
    public var isWhoop5Family: Bool { self == .whoop5mg }
    ...
    /// Never invent — returns nil when the name is missing or not a WHOOP. Mirrors Android.
    public static func fromBleName(_ name: String?) -> WhoopModel? {
        let n = (name ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        guard !n.isEmpty else { return nil }
        guard n.uppercased().hasPrefix("WHOOP") else { return nil }
        if n.uppercased().hasPrefix("WHOOP 4") { return .whoop4 }
        if n.uppercased().hasPrefix("WHOOP 3") { return .whoop4 } // classic Harvard-era → 4 framing
        return .whoop5mg
    }
    ...
    public static func resolveForEstimates(
        selected: WhoopModel,
        whoop5Detected: Bool,
        persisted: WhoopModel?
    ) -> WhoopModel {
        if whoop5Detected { return .whoop5mg }
        if selected.isWhoop5Family { return .whoop5mg }
        if persisted?.isWhoop5Family == true { return .whoop5mg }
        return selected
    }
