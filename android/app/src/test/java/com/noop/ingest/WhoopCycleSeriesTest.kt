...
class WhoopCycleSeriesTest {
    ...
    @Test
    fun blankPerformanceUsesAsleepOverNeedProxy() {
        // Fable Rest #21: when Sleep performance % is blank but asleep + need exist, bank a proxy.
        val csv = """
            Cycle start time,Cycle end time,Cycle timezone,Asleep duration (min),Sleep performance %,Sleep consistency %,Sleep need (min),Sleep debt (min)
            2026-06-01 22:30:00,2026-06-02 21:00:00,UTC+02:00,420,,88,480,60
        """.trimIndent().toByteArray()
        val rows = WhoopCsvImporter.parseCycleSeries(CsvTable.fromData(csv), "my-whoop")
        assertEquals(87.5, rows.first { it.key == "sleep_performance" }.value, 1e-9) // 420/480*100
        assertEquals(87.5, rows.first { it.key == "hours_vs_needed_pct" }.value, 1e-9)
        assertEquals(88.0, rows.first { it.key == "sleep_consistency" }.value, 1e-9)
        assertEquals(480.0, rows.first { it.key == "sleep_need_min" }.value, 1e-9)
    }

    @Test
    fun blankPerformanceCapsAtScoreContractWhileHoursNeededShowsOversleep() {
        val csv = """
            Cycle start time,Cycle end time,Cycle timezone,Asleep duration (min),Sleep performance %,Sleep need (min)
            2026-06-01 22:30:00,2026-06-02 21:00:00,UTC+02:00,600,,480
        """.trimIndent().toByteArray()
        val rows = WhoopCsvImporter.parseCycleSeries(CsvTable.fromData(csv), "my-whoop")
        assertEquals(100.0, rows.first { it.key == "sleep_performance" }.value, 1e-9)
        assertEquals(125.0, rows.first { it.key == "hours_vs_needed_pct" }.value, 1e-9)
    }
