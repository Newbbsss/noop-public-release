...
class WatchRecoveryTest {
    ...
    /** Fable Rest #8: gate is Baselines.minNightsSeed (4), not a separate week-long null. */
    @Test
    fun historyJustBelowSeedGateCalibrates() {
        val n = WatchRecovery.minBaselineNights - 1
        val out = WatchRecovery.compute(
            todayHrv = 45.0, todayRhr = 52,
            hrvHistory = List(n) { 45.0 },
            rhrHistory = List(n) { 52.0 },
        )
        assertNull(out.recovery)
        assertEquals(ScoreConfidence.CALIBRATING, out.confidence)
    }

    /** At the shared seed gate Charge scores; thin baseline → BUILDING, not blank. */
    @Test
    fun historyAtSeedGateScoresBuilding() {
        val n = WatchRecovery.minBaselineNights
        assertEquals(Baselines.minNightsSeed, n)
        val out = WatchRecovery.compute(
            todayHrv = 45.0, todayRhr = 52,
            hrvHistory = List(n) { 45.0 },
            rhrHistory = List(n) { 52.0 },
        ...
        assertNotNull(out.recovery)
        assertEquals(ScoreConfidence.BUILDING, out.confidence)
    }
