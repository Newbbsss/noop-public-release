...
class FramingTest {
    ...
    @Test
    fun extractStreams_dropsOutOfRangeHrAndRr() {
        // Realtime frame with HR=62 and rr=[850,870] — mutate parse map to spike values.
        val base = Framing.parseFrame(
            bytes(
                0xaa, 0x12, 0x00, 0x7d, 0x28, 0x00, 0x00, 0xf1, 0x53, 0x65, 0x00, 0x00,
                0x3e, 0x02, 0x52, 0x03, 0x66, 0x03, 0x73, 0x8f, 0x40, 0xae,
            ),
        )
        val spiked = base.copy(
            parsed = base.parsed.toMutableMap().apply {
                put("heart_rate", 250)
                put("rr_intervals", listOf(100, 800, 5000))
            },
        )
        val streams = extractStreams(listOf(spiked), deviceClockRef = 1700000000, wallClockRef = 1700000000)
        assertTrue(streams.hr.isEmpty())
        assertEquals(1, streams.rr.size)
        assertEquals(800, streams.rr[0].rrMs)
    }
