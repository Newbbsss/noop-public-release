...
object WhoopCsvImporter {
    ...
    suspend fun importZip(
        ...
        // Data Export — not open BLE and not NOOP-computed Effort. Compare card reads ONLY this id.
        if (daily.isNotEmpty()) {
            val appId = com.noop.data.WhoopAppScoreStore.DEVICE_ID
            repo.upsertDevice(appId, name = "WHOOP app (export)")
            val native = daily.map { d ->
                // Reverse Effort rescale if present: importer stored strain as *100/21 for my-whoop.
                val strain021 = d.strain?.let { s ->
                    if (s > 21.0 + 1e-6) s * 21.0 / 100.0 else s
                }
                d.copy(deviceId = appId, strain = strain021)
            }
            repo.upsertDailyMetrics(native)
        }
    ...
    private fun loadCsvData(context: Context, uri: Uri): Map<String, ByteArray> {
        ...
        val firstBytes: ByteArray = context.contentResolver.openInputStream(uri)?.use { it.readAllCapped(MAX_ENTRY_BYTES) }
            ?: uri.path?.let { path ->
                java.io.File(path).takeIf { it.isFile }?.inputStream()?.use { it.readAllCapped(MAX_ENTRY_BYTES) }
            }
    ...
    internal fun parseCycleSeries(table: CsvTable, deviceId: String): List<MetricSeriesRow> {
        ...
        for (row in table.rows) {
            ...
            // Verbatim sleep_performance_pct when present; else asleep/need proxy so Rest still
            // banks from exports that omit the % column (Fable Rest #21). Never invents nights.
            val asleepMin = row.double("asleep_duration_min")
            val needMin = row.double("sleep_need_min")
            val perf = row.double("sleep_performance_pct")
                ?: if (asleepMin != null && needMin != null && needMin > 0.0) {
                    // sleep_performance is a 0–100 score. Keep catch-up sleep above need visible in
                    // hours_vs_needed_pct below, but never leak that >100 ratio into the Rest score.
                    (asleepMin / needMin * 100.0).coerceIn(0.0, 100.0)
                } else {
                    null
                }
            add("sleep_performance", perf)
            add("sleep_consistency", row.double("sleep_consistency_pct"))
            add("sleep_need_min", needMin)
            add("sleep_debt_min", row.double("sleep_debt_min"))
            if (asleepMin != null && needMin != null && needMin > 0.0) {
                add("hours_vs_needed_pct", asleepMin / needMin * 100.0)
            }
        }
        return out
    }
