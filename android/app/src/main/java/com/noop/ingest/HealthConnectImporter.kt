    ...
    private fun hcStagesJson(r: SleepSessionRecord): String? {
        ...
        for (s in r.stages) {
            ...
            when (s.stage) {
                ...
                // Generic "asleep" with no Deep/REM split (WHOOP-via-HC often lands here). Map to
                // light so Sleep chart + NOOP Asleep populate — never invent Deep/REM.
                SleepSessionRecord.STAGE_TYPE_SLEEPING -> light += min
                else -> {}   // UNKNOWN: counted in totalSleepMin only
            }
        }
