...
object AndroidDiagnostics {
    ...
    suspend fun strapAndDataLines(context: Context): List<String> = buildList {
        ...
        runCatching {
            ...
            // Fable Rest #29: Charge (recovery) vs Rest (sleep_performance) side-by-side so a
            // "calibrating forever" report shows which series is actually banked.
            val restSeries = repo.resolvedSeries(
                "sleep_performance", "my-whoop", "0000-00-00", "9999-99-99",
            ).points
            val lastRest = restSeries.lastOrNull()
            add(
                "Last Rest:   ${
                    lastRest?.let { "${it.day} · ${it.value.toInt()}%" } ?: "none"
                }",
            )
            val recentDays = days.takeLast(7)
            if (recentDays.isNotEmpty()) {
                val n = recentDays.size
                val restDays = restSeries.map { it.day }.toHashSet()
                add(
                    "Recent ${n}d Charge vs Rest: " +
                        "recovery=${recentDays.count { it.recovery != null }}/$n  " +
                        "sleep_performance=${recentDays.count { it.day in restDays }}/$n",
                )
            }
    ...
    suspend fun dailyDataLines(context: Context): List<String> = buildList {
        ...
        runCatching {
            ...
            if (recent.isNotEmpty()) {
                ...
                // Fable Rest #29: sleep_performance coverage next to recovery so Rest gaps show up.
                val restDays = repo.resolvedSeries(
                    "sleep_performance", "my-whoop",
                    recent.first().day, recent.last().day,
                ).points.map { it.day }.toHashSet()
                add(
                    "Recent ${n}d Rest series: sleep_performance=${recent.count { it.day in restDays }}/$n " +
                        "(vs recovery=${recent.count { it.recovery != null }}/$n)",
                )
