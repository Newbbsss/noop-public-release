...
object AnalyticsEngine {
    ...
    fun analyzeDay(
        ...
        // Dual HRV: overnight SDNN (same weighting as RMSSD). Null when no session carried SDNN.
        val avgSdnnDaily: Double? = run {
            val pairs = matched.mapNotNull { s ->
                s.avgSdnn?.let { it to (s.end - s.start).toDouble() }
            }
            if (pairs.isEmpty()) {
                null
            } else {
        ...
        val stepsTotal: Int? = run {
            ...
            // Same accumulator used by Step Training. A labelled session therefore learns against the
            // identical accepted-tick quantity that daily analytics converts into estimated steps.
            val total = StepMotionCounter.accumulatePersisted(
                sorted,
                mode = StepMotionCounter.Mode.Production,
                noiseFloorTicksPerSec = profile.noiseFloorTicksPerSec.coerceIn(0.0, 2.5),
            ).acceptedTicks
            if (total <= 0.0) return@run null
            // @57 overcounts vs true footfalls — user-calibrated ticks-per-step (default 1.0).
            // The trainer and Settings both support 0.5..30. Never silently clamp a saved coefficient
            // differently here, otherwise calibration appears to save but cannot affect daily estimates.
            val ticksPer = profile.stepTicksPerStep.coerceIn(0.5, 30.0)
            val scaled = (total / ticksPer).roundToLong()
...
object RestScorer {
     ...
     *  low-deep night reads honestly without the whole night tanking. Gilbert Fold: Rest ~10 over WHOOP. */
    const val deepFloorFactor: Double = 0.14
     ...
     */
    const val durationQualityShareGate: Double = 0.16
     ...
     * the root of Sleep hero disagreeing with stages on the same night. Never invents stages.
     */
    ...
    fun rest(
        ...
        val aligned = (weighted * restWhoopAlignScale).coerceIn(0.0, 100.0)
        return (aligned * 100.0).roundToInt() / 100.0
    }
    ...
    /**
     * @param sleepNeedHours personal need (hours); null → [defaultSleepNeedHours].
     *   Callers with history should pass [personalNeedHours] so duration matches Sleep's need.
     */
    fun restFromDaily(
        daily: DailyMetric,
        consistency: Double? = null,
        ...
        // Stage sum preferred so Rest cannot disagree with hypnogram cards on the same night.
        val tstMin = canonicalAsleepMin(daily) ?: return null
        if (tstMin <= 0.0) return null
        // Bare aggregate (HC / import without efficiency): duration-only Rest proxy — never invent Deep/REM.
        // Duration proxy is vs the same personal need the full composite uses (Fable Rest #24).
        val eff = daily.efficiency
            ?: return HcNoopAlign.durationAsSleepPerf(tstMin, sleepNeedHours)?.coerceIn(0.0, 100.0)
        return rest(
