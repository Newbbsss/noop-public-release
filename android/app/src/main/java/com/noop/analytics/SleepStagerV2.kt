...
object SleepStagerV2 {
    ...
    /** Population sleep-architecture base rates as log-priors. DREAMT (#348) retune raises awake prior
     *  (waso-heavy clinical cohort) and slightly lowers deep prior so light wins weak-evidence epochs. */
    private val baseLogPrior: Map<String, Double> = mapOf(
        "light" to ln(0.50), "deep" to ln(0.15), "rem" to ln(0.22), "awake" to ln(0.34))

    /** Deep-eligibility HR-flatness percentile gate. Raised → 0.40 by DREAMT (n=100) joint re-tune
     *  (#348 / ryanbr): deep only in the flattest epochs; held-out kappa/wake-recall improved. */
     ...
     *  DREAMT re-tune: move 38→75 (stricter), gate 55→35 (wake fires easier), boost 2→4. */
    ...
    /** Deadzone on awake HR z-scores so mild cardiac noise does not open WASO (#348). */
    ...
    /** Transition matrix (rows = from, cols = to). DREAMT (#348) sticky wake + looser deep↔light. */
    ...
    private fun stageEpochs(feats: List<Epoch>): List<String> {
        ...
        for (f in feats) {
            ...
            // Emission weights from DREAMT (#348) joint re-tune (ryanbr SleepStagerV2).
