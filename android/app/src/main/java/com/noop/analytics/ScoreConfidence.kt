...
enum class ScoreConfidence(val raw: String) {
    ...
    companion object {
        ...
        fun forRest(
            ...
            // #395 / #345: sparse-motion staging cannot earn SOLID Rest (confidence-only; no score invent).
            if (gravitySparse) return BUILDING
        ...
        /**
         * Rest confidence from a banked [com.noop.data.DailyMetric] row (Fable Rest #12).
         * Same H9 rule as the engine's daily pass — surfaces the tier on Sleep hero without
         * inventing a second decision. [hasSession] is true when a matched in-bed session
         * exists for the navigated night (even if totals are still thin).
         */
        fun forRestFromDaily(
            ...
            val session = hasSession || asleepMin > 0.0 || hasStaged
            if (!session) return CALIBRATING
            val rawEff = daily?.efficiency
            val efficiency = when {
                rawEff == null -> 0.0
