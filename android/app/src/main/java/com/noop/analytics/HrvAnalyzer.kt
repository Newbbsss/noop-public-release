...
object HrvAnalyzer {
     ...
     */
    fun rejectEctopic(nn: List<Double>, threshold: Double = ECTOPIC_THRESHOLD): List<Double> {
    ...
    fun cleanRR(rr: List<Double>, ectopicThreshold: Double = ECTOPIC_THRESHOLD): List<Double> =
        rejectEctopic(rangeFilter(rr), threshold = ectopicThreshold)
    ...
    fun analyzeRaw(
        ...
        ectopicThreshold: Double? = null,
    ): HrvResult {
        val nInput = rawRR.size
        val clean = cleanRR(rawRR, ectopicThreshold = ectopicThreshold ?: ECTOPIC_THRESHOLD)
    ...
    /**
     * Membership-free “feel” RMSSD from a short live R-R bank (type-40 / 0x2A37).
     * Needs ≥3 intervals; not a clinical overnight figure — Health / Today live chips only.
     */
    fun feelRmssdMs(rrRecentMs: List<Int>, takeLast: Int = 12): Double? {
        val values = rrRecentMs.takeLast(takeLast)
        if (values.size < 3) return null
        return rmssdRaw(values.map { it.toDouble() })
    }
