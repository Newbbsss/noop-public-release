...
object SleepStageHealer {
    ...
    /**
     * Fable Rest #20 — build sleep-derived [DailyMetric] fields from restaged `stagesJSON` so Rest can
     * recompute without raw streams. Pure; null when stages decode to no asleep minutes.
     * Never invents Deep/REM beyond what the JSON already carries.
     */
    fun dailyFromRestagedStages(
        deviceId: String,
        day: String,
        stagesJSON: String?,
    ): DailyMetric? {
        val mins = SleepStageTotals.minutes(stagesJSON) ?: return null
        if (mins.asleep <= 0.0) return null
        val eff = if (mins.inBed > 0.0) (mins.asleep / mins.inBed).coerceIn(0.0, 1.0) else null
        return DailyMetric(
            deviceId = deviceId,
            day = day,
            totalSleepMin = mins.asleep,
            efficiency = eff,
            deepMin = mins.deep.takeIf { it > 0.0 },
            remMin = mins.rem.takeIf { it > 0.0 },
            lightMin = mins.light.takeIf { it > 0.0 },
        )
    }
