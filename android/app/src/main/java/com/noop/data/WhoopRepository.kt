...
class WhoopRepository(private val dao: WhoopDao) {
    ...
    /** Sleep-bank fingerprint `"count:maxEndTs"` — when HC / strap night lands, Charge+Rest must
     *  rescore even if raw HR watermark is unchanged (recovery was ~20 pts stale vs WHOOP). */
    suspend fun sleepFingerprint(): String = "${dao.countSleepSessions()}:${dao.maxSleepEndTs()}"
    ...
    /** Range-delete one computed metric key over [from, to] (inclusive day keys) before re-upserting,
     *  so the recompute FULLY OWNS its series in the window (Fable #201: a stale `sleep_performance`
     *  point on a day whose night no longer scores was pinned forever by the upsert-only path). */
    suspend fun deleteComputedSeriesInRange(deviceId: String, key: String, from: String, to: String) =
        dao.deleteMetricSeriesInRange(deviceId, key, from, to)
    ...
    suspend fun updateSleepStages(deviceId: String, detectedStartTs: Long, stagesJSON: String): Int {
        // Fable Rest #20: keep session efficiency in lockstep with restaged stages so Sleep + Rest
        // don't disagree after self-heal (stages-only UPDATE used to leave a stale efficiency).
        val efficiency = sleepEfficiency(stagesJSON)
        return dao.updateSleepStages(deviceId, detectedStartTs, stagesJSON, efficiency)
    }
    ...
    /**
     * Delete workouts owned by one computed provenance in [from, to]. Sport is an inferred display
     * label and may change between runs, so it cannot safely identify the engine's replace-set.
     */
    suspend fun deleteComputedWorkoutsBySource(deviceId: String, source: String, from: Long, to: Long) =
        dao.deleteWorkoutsBySource(deviceId, source, from, to)
