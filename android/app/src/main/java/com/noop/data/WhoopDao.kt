...
interface WhoopDao : DeviceRegistryDao {
    ...
    /**
     * Range-delete ONE metric key for a source across a day range (inclusive, yyyy-MM-dd). Fable #201
     * recompute hygiene: the engine rebuilds the computed `sleep_performance` series every pass
     * (delete-then-upsert, like [deleteDailyMetricsInRange]) so a day whose night no longer scores
     * drops its stale point instead of pinning it forever , upsert-only left the cross-day-repeat
     * rows poisoning Rest even after the aggregate bug was fixed. Source-scoped: imported rows and
     * every other key are never touched.
     */
    @Query("DELETE FROM metricSeries WHERE deviceId = :deviceId AND key = :key AND day >= :from AND day <= :to")
    suspend fun deleteMetricSeriesInRange(deviceId: String, key: String, from: String, to: String)
    ...
    /** Delete engine-owned workouts by durable provenance rather than the mutable/inferred sport label. */
    @Query("DELETE FROM workout WHERE deviceId = :deviceId AND source = :source AND startTs >= :from AND startTs <= :to")
    suspend fun deleteWorkoutsBySource(deviceId: String, source: String, from: Long, to: Long)
    ...
    /** Sleep-bank fingerprint so Charge/Rest rescore when a night lands even if raw HR is unchanged. */
    @Query("SELECT COUNT(*) FROM sleepSession") suspend fun countSleepSessions(): Int
    @Query("SELECT COALESCE(MAX(endTs), 0) FROM sleepSession") suspend fun maxSleepEndTs(): Long
