...
abstract class WhoopDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** v18: dual HRV — persist overnight SDNN beside RMSSD on dailyMetric (nullable). */
        internal val DAILY_AVG_SDNN_MIGRATION_SQL: List<String> = listOf(
            "ALTER TABLE `dailyMetric` ADD COLUMN `avgSdnn` REAL",
        )

        internal val MIGRATION_17_18 = object : Migration(17, 18) {
            override fun migrate(db: SupportSQLiteDatabase) {
                for (stmt in DAILY_AVG_SDNN_MIGRATION_SQL) db.execSQL(stmt)
            }
        }
