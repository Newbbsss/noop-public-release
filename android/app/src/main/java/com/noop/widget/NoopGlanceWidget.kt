...
// Glass-leaning M3 expressive surfaces (translucent navy / paper) for home widgets.
private fun widgetSurface(dark: Boolean) = ColorProvider(if (dark) Color(0xE60E1828) else Color(0xF0F7F4EE))
...
private fun WidgetContent(snap: WidgetSnapshot, dark: Boolean) {
    ...
    Column(
        ...
        // Fable Stress #43 — Now tip when scored (honest-null otherwise).
        snap.stressTipTenths?.let { tenths ->
            val tip = tenths / 10.0
            Text(
                text = String.format(java.util.Locale.US, "Stress %.1f", tip),
                style = TextStyle(color = textSecondary, fontSize = 11.sp),
            )
            Spacer(modifier = GlanceModifier.height(2.dp))
        }
        // ALARM_PAGE #98 — next armed wake when present.
        snap.nextAlarmLabel?.let { label ->
            Text(
                text = label,
                style = TextStyle(color = textSecondary, fontSize = 11.sp),
            )
