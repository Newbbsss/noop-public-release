...
data class WidgetSnapshot(
    ...
    /**
     * Daytime Stress Now tip ×10 (0–30 for 0.0–3.0), null until [DaytimeStress] scores today
     * (Fable Stress #43). Honest-null — never invents a tip.
     */
    val stressTipTenths: Int? = null,
    /** Short next-wake line when an alarm is armed (ALARM_PAGE #98); null when nothing armed. */
...
object WidgetSnapshotStore {
    ...
    fun save(context: Context, snap: WidgetSnapshot) {
        context.getSharedPreferences(FILE, Context.MODE_PRIVATE).edit()
            ...
            .putInt("stressTipTenths", snap.stressTipTenths ?: -1)
            .putString("nextAlarmLabel", snap.nextAlarmLabel)
            .apply()
    }
    ...
    fun load(context: Context): WidgetSnapshot {
        ...
        return WidgetSnapshot(
            ...
            stressTipTenths = p.getInt("stressTipTenths", -1).takeIf { it >= 0 },
            nextAlarmLabel = p.getString("nextAlarmLabel", null)?.takeIf { it.isNotBlank() },
        )
    }
...
internal object PushGate {
    ...
    private fun keyOf(snap: WidgetSnapshot): String =
        ...
        // same way recovery does — not waiting out the HR refresh window. Stress tip (#43) likewise.
        "${snap.recoveryPct}|${snap.restPct}|${snap.effortPct}|${snap.stressTipTenths}|" +
            "${snap.nextAlarmLabel}|${snap.batteryPct?.div(5)}|${snap.connected}|${snap.heartRate != null}"
