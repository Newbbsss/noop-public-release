...
class SmartAlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        val firedDeadlineMs = store.scheduledDeadlineMs
        ...
        // the smart alarm recurs each morning. An early smart cue can fire while today's original
        // deadline is still ahead, so recurrence must be anchored after that deadline, not after now.
        // A disabled-but-fired alarm does NOT resurrect itself.
        runCatching {
            if (store.enabled) {
                SmartAlarmScheduler.armNextAfterFired(context, store, firedDeadlineMs)
            }
        }
