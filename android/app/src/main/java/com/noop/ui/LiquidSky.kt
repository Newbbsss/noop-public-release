...
private fun androidx.compose.ui.graphics.drawscope.DrawScope.renderLiquidSky(
    ...
    val top = paperSkyTone(s.top, light)
    val mid = paperSkyTone(s.mid, light)
    val hor = paperSkyTone(s.hor, light)
    val w = size.width
    val h = size.height
    ...
    drawRect(
        brush = Brush.verticalGradient(
            colors = listOf(
                ...
                settle.copy(alpha = settleAlpha),
            ),
            startY = h * settleStart,
            endY = h,
        ),
