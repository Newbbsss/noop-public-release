...
enum class KeyMetric(val raw: String, val title: String) {
    ...
    companion object {
        ...
        /**
         * Default Today Key Metrics — Charge/Effort/Rest + Stress/FA/Vitality + activity.
         * HRV / RHR / Resp / SpO₂ stay available in the editor but default OFF so Health owns
         * those vitals chips (SHIP 145 leftover — no duplicate clinical strip on Today).
         */
        ...
        /** Vitals tiles Health owns — offered in the editor, not on by default. */
        val healthOwnedOptional: List<KeyMetric> = listOf(
            HRV, RESTING_HR, BLOOD_OXYGEN, RESPIRATORY,
        )
    }
}
