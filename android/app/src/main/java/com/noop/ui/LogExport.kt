...
object LogExport {
    ...
    /**
     * Build the shareable EVENT-frame log (rotated + live `whoop5-events.jsonl`, upstream PR #346)
     * under cache/logs, or null when none has accumulated. EVENT (type 48) frames are the rare,
     * still-uncatalogued 5/MG records (e.g. the nightly sleep-SpO2 measurement cycle) — the matched
     * pair export bundles them so a decode report arrives with weeks of them, not just the current
     * session's bulk capture.
     */
    private fun writeEventLogFile(context: Context): File? {
        val main = File(context.filesDir, com.noop.ble.WhoopBleClient.WHOOP5_EVENT_LOG_FILE)
        val prev = File(context.filesDir, "${com.noop.ble.WhoopBleClient.WHOOP5_EVENT_LOG_FILE}.1")
        if (!main.exists() && !prev.exists()) return null
        val header = buildString {
            appendLine("# NOOP 5/MG EVENT-frame log (JSONL; {\"ts_ms\",\"char\",\"hex\"} per line; type 48 / 0x30 frames only)")
            appendLine("# App: ${BuildConfig.VERSION_NAME} (${BuildConfig.TIER}) · Android ${Build.VERSION.RELEASE} (SDK ${Build.VERSION.SDK_INT}) · ${Build.MANUFACTURER} ${Build.MODEL}")
        }
        val dir = File(context.cacheDir, "logs").apply { mkdirs() }
        val out = File(dir, "noop-event-log-${timestamp()}.jsonl")
        out.outputStream().bufferedWriter().use { w ->
            w.write(header)
            for (f in listOf(prev, main)) if (f.exists()) f.bufferedReader().use { r -> r.copyTo(w) }
        }
    ...
    /**
     * Build the shareable high-rate deep-buffer log (`whoop5-deepbuffers.jsonl`, #423/#476) — 1244-B
     * 6-axis IMU (+ inline `"imu":{…}` decode) and 2140-B optical buffers. Null when none accumulated.
     */
    private fun writeDeepBufferLogFile(context: Context): File? {
        ...
        if (!main.exists() && !prev.exists()) return null
        val header = buildString {
            appendLine("# NOOP 5/MG high-rate deep-buffer log (JSONL; type-0x2F ≥1 KB; 1244-B IMU may include \"imu\" features)")
            appendLine("# Live TOGGLE_IMU_MODE/106 ACKs but does not stream; IMU arrives via historical offload only.")
            appendLine("# App: ${BuildConfig.VERSION_NAME} (${BuildConfig.TIER}) · Android ${Build.VERSION.RELEASE} (SDK ${Build.VERSION.SDK_INT}) · ${Build.MANUFACTURER} ${Build.MODEL}")
        }
        val dir = File(context.cacheDir, "logs").apply { mkdirs() }
        val out = File(dir, "noop-deepbuffers-${timestamp()}.jsonl")
        out.outputStream().bufferedWriter().use { w ->
            w.write(header)
            for (f in listOf(prev, main)) if (f.exists()) f.bufferedReader().use { r -> r.copyTo(w) }
        }
    ...
    suspend fun shareRawAndLog(context: Context, logText: String, whoop5Connected: Boolean) {
        runCatching {
            ...
            // The durable EVENT-frame log rides along whenever it exists (upstream PR #346): the
            // weeks-long record the decode effort actually needs, tiny next to the bulk capture.
            writeEventLogFile(context)?.let { entries.add("events.jsonl" to it.readBytes()) }
            // High-rate IMU/optical deep buffers (#423/#476) — 1244-B lines carry inline decoded imu features.
            writeDeepBufferLogFile(context)?.let { entries.add("deepbuffers.jsonl" to it.readBytes()) }
