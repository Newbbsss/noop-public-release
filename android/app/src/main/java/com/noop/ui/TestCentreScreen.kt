...
import com.noop.testcentre.AndroidDiagnostics
...
fun TestCentreScreen(vm: AppViewModel) {
    ...
    // Re-read each composition — freezing in remember {} left the 5/MG Advanced gate stale after pair.
    val selectedModelName = NoopPrefs.of(context).getString("noop.selectedWhoopModel", null)
    ...
    ScreenScaffold(
        ...
        subtitle = LifeChapterLacquer.TEST_CENTRE_SUBTITLE,
    ) {
        // Preview motion / overlays without a live strap (emulator UI iteration).
        // DEBUG-only lab — algo-vs-WHOOP compare ships on Today + More → NOOP vs WHOOP.
        if (BuildConfig.DEBUG) {
            UiDemoLabSection()
        }

        // 6-axis motion tester (MAIN + DEBUG) — band IMU only; offload labeled not live; no phone.
        SettingsSectionTC(
            icon = Icons.Filled.Sensors,
            title = "6-axis motion tester",
            blurb = "Band IMU only. Type-51 live when it streams; offload not-live; waiting while gated — no phone.",
        ) {
            SixAxisMotionDotTester(ble = vm.ble, compact = true)
        }
...
private fun DiagnosticToolsCard(vm: AppViewModel) {
    ...
    SettingsSectionTC(
        ...
        Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            // Environment dump: clipboard of AndroidDiagnostics (+ dynamic), not a twin Share.
            NoopButton(
                ...
                onClick = {
                    scope.launch {
                        val text = buildString {
                            ...
                            for (line in AndroidDiagnostics.dynamicLines(context)) appendLine(line)
                        }
                        val cm = context.getSystemService(android.content.Context.CLIPBOARD_SERVICE)
                            as android.content.ClipboardManager
                        cm.setPrimaryClip(ClipData.newPlainText("NOOP environment", text))
                        Toast.makeText(context, "Environment dump copied.", Toast.LENGTH_SHORT).show()
                    }
                },
            )
        }
