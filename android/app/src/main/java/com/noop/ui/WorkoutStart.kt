...
fun StartWorkoutSheet(vm: AppViewModel, onDismiss: () -> Unit) {
    ...
    // Fable 200 #54 — first start asks for sensor disclosure (HR / optional GPS).
    var sensorOk by remember {
        mutableStateOf(NoopPrefs.workoutSensorDisclosureSeen(context))
    }
    ...
    AlertDialog(
        ...
        text = {
            Column {
                ...
                if (!NoopPrefs.workoutSensorDisclosureSeen(context)) {
                    Row(
                        verticalAlignment = Alignment.CenterVertically,
                        ...
                        Switch(
                            checked = sensorOk,
                            onCheckedChange = { sensorOk = it },
                        )
                        Spacer(Modifier.width(8.dp))
                        Text(
                            "I understand NOOP uses strap heart rate (and GPS if on) on this device, " +
                                "and may use phone motion / IMU for sport hints — never sent off-device.",
        ...
        confirmButton = {
            Button(
                onClick = {
                    if (!sensorOk) return@Button
                    NoopPrefs.setWorkoutSensorDisclosureSeen(context)
                    if (gpsOn) {
                        startWithGps()
                    } else {
...
fun WorkoutStartSection(vm: AppViewModel) {
    ...
    if (w != null) {
        ...
        ConfirmEndWorkout(
            visible = confirmEnd,
            onConfirm = { confirmEnd = false; vm.endWorkout() },
...
/**
 * Shared end-workout confirm (#151) — LiveWorkout Done, Live End workout, WorkoutStartSection End.
 * One extra tap is OK for density ship; keeps accidental ends rare.
 */
@Composable
fun ConfirmEndWorkout(
    visible: Boolean,
    onConfirm: () -> Unit,
    onDismiss: () -> Unit,
) {
    ...
    AlertDialog(
        ...
        confirmButton = {
            Button(
                onClick = onConfirm,
                colors = ButtonDefaults.buttonColors(
                    containerColor = Palette.statusCritical,
                    contentColor = Palette.surfaceBase,
                ),
            ) { Text("End workout") }
        },
        dismissButton = {
            OutlinedButton(onClick = onDismiss) { Text("Keep going") }
        },
    )
}
