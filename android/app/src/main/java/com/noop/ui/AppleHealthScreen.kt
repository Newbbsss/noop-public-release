...
@Composable
fun AppleHealthScreen(
    vm: AppViewModel,
    onOpenDataSources: (() -> Unit)? = null,
) {
    ...
    LazyScreenScaffold(title = "Apple Health", subtitle = subtitle) {
        when {
            ...
            !data.hasAnyData -> item { EmptyState(onOpenDataSources = onOpenDataSources) }
            else -> {
...
@Composable
private fun EmptyState(onOpenDataSources: (() -> Unit)? = null) {
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        DataPendingNote(
            title = LifeChapterLacquer.APPLE_HEALTH_EMPTY_TITLE,
            body = "Nothing imported yet. On an iPhone: Health app, tap your photo, Export " +
                "All Health Data, then import the .zip in Data Sources.",
        )
        if (onOpenDataSources != null) {
            NoopButton(
                text = LifeChapterLacquer.APPLE_HEALTH_OPEN_DATA_SOURCES,
