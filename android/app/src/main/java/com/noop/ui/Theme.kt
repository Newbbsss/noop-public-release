...
object Palette {
     ...
     * Set by [NoopTheme] — must NOT use `active === LightTokens`, because theme packs assign
     * `LightTokens.copy(...)` (a different instance) and that broke every light-pack branch.
     */
    internal var schemeIsLight by mutableStateOf(false)
    val isLight: Boolean get() = schemeIsLight
...
object Metrics {
    ...
    val space18 = 18.dp
    val space24 = 24.dp
    val cardRadius = 24.dp   // Bubblier continuous radius (was 18; 22–26 reads softer)
    val cornerXs = 2.dp
    val cornerSm = 12.dp
...
private val NoopShapes = Shapes(
    ...
    large = RoundedCornerShape(Metrics.cardRadius), // SHIP #239 — same 24dp as cards post-Cycle redo
...
fun NoopTheme(content: @Composable () -> Unit) {
    ...
    val pack = ThemePackPrefs.current
    val tokens = if (dark) pack.dark else pack.light
    if (Palette.schemeIsLight != !dark) Palette.schemeIsLight = !dark
