...
object TrendsReportRenderer {
    ...
    private fun valueText(v: Double, metric: ReportMetric): String {
        // One decimal for sleep hours, respiratory rate, skin-temp delta and the 0-3 stress score
        ...
        // PDF Helvetica lacks degree / fancy glyphs — ASCII-sanitize the unit suffix.
        val unit = metric.unit.replace("°", "").replace("·", "|")
...
fun TrendsReportExportSection(vm: AppViewModel, modifier: Modifier = Modifier) {
    ...
    // Stored daily stress series ("yyyy-MM-dd" → 0–3) for the Stress row (#457 / Fable #321).
    // Same classic∪daytime union Stress Monitor uses — not only `my-whoop`/`stress`.
