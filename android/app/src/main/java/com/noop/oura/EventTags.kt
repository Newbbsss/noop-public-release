...
enum class OuraEventTag(val raw: Int) {
    ...
    val tier: TrustTier
        get() = when (this) {
            ...
            // #287 (upstream 8.7.0): 0x71 green_ibi_and_amp is NOT corpus-verified — no captured 0x71
            // fixture, and OURA_PROTOCOL.md §6.2 documents a DIFFERENT layout (5 IBI deltas + 6
            // amplitudes, shift [2:0]) than the 0x60 decoder it was wired to (6 absolute IBIs, 4-bit
            // shift). Decoding it with the 0x60 layout fabricates a 6th phantom R-R and reads deltas as
            // absolute intervals, silently corrupting reconstructed HRV. Demote to Tier B (gated out of
            // live emission) until a real 0x71 capture lets us write + verify a dedicated decoder.
            // (TIER_A == corpus-verified.)
