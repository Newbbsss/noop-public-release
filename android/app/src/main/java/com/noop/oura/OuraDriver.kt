    ...
    fun ingest(record: OuraRecord): List<OuraEvent> {
        ...
        return when (tag) {
            ...
            OuraEventTag.GREEN_IBI_AMP ->
                // #287 (upstream 8.7.0): 0x71 green_ibi_and_amp. Demoted from Tier A: the 0x60 decoder it
                // used reads 6 ABSOLUTE IBIs, but §6.2 documents 0x71 as 5 IBI DELTAS + 6 amplitudes with
                // a [2:0] shift, so that decode fabricated a phantom R-R and corrupted HRV. With no
                // captured 0x71 fixture we cannot write a verified decoder yet, so emit the raw bytes for
                // inspection (never folded into scoring) rather than a guessed IBI. Gated above unless
                // allowTierB. Promote on a real sample.
                listOf(
                    OuraEvent.TierB(
                        OuraTierBSummary(
