...
class AiCoach(private val repo: WhoopRepository) {
     ...
     * Gemini: chat-capable gemini-* only; Anthropic: all returned ids) and de-duplicated.
     *
    ...
    suspend fun fetchModels(
        ...
        runCatching {
            ...
            if (provider == AiProvider.GEMINI) {
                return@runCatching parseGeminiModels(json)
            }

            // OpenAI-shaped providers (incl. Custom) return {"data": [ { "id": "..." }, ... ]}.
            val data = json.optJSONArray("data") ?: return@runCatching emptyList<String>()
            ...
            for (i in 0 until data.length()) {
                ...
                val keep = when (provider) {
                    ...
                    AiProvider.GEMINI -> false // handled above
                }
                if (keep) ids.add(id)
            }
            ids.distinct()
    ...
    private fun callGemini(
        ...
        // Gemini 2.5 counts THINKING tokens against maxOutputTokens; a small cap starves
        // thinking models into empty replies. 4096 leaves room; the system prompt keeps replies short.
        val body = JSONObject()
        ...
        val url = "${provider.endpoint.trimEnd('/')}/${model}:generateContent"
        ...
        if (parts == null || parts.length() == 0) {
            throw Exception("The provider returned an empty reply. Please try again.")
        }
        val joined = buildString {
            for (i in 0 until parts.length()) {
                val t = parts.optJSONObject(i)?.optString("text")?.trim().orEmpty()
                if (t.isNotEmpty()) append(t)
            }
        }.trim()
        if (joined.isEmpty()) throw Exception("The provider returned an empty reply. Please try again.")
        return joined
    }
    ...
    companion object {
        ...
        /**
         * Unwrap Gemini's `{"models":[{"name":"models/…"}]}` — strip the prefix, keep chat-capable
         * gemini-* only (drop embeddings/AQA). Pure — unit-tested; no network.
         */
        fun parseGeminiModels(json: JSONObject): List<String> {
            val list = json.optJSONArray("models") ?: return emptyList()
            ...
            for (i in 0 until list.length()) {
                ...
                if (id.contains("embedding") || id.contains("aqa")) continue
                ids.add(id)
            }
            return ids.distinct()
        }
