...
object Framing {
    ...
    private fun decodeRealtimeWhoop5(frame: ByteArray, parsed: MutableMap<String, Any?>) {
        ...
        // Cap rr_count to bytes remaining after rr@18 so a corrupt count cannot walk past the frame.
        val rrnRaw = frame.u8(17) ?: 0
        val rrn = minOf(rrnRaw, ((frame.size - 18) / 2).coerceAtLeast(0))
        parsed["rr_count"] = rrn
    ...
    private fun decodeRealtime(frame: ByteArray, parsed: MutableMap<String, Any?>) {
        ...
        // Cap rr_count to bytes remaining after rr@14 (corrupt/truncated type-40 safety).
        val rrnRaw = frame.u8(13) ?: 0
        val rrn = minOf(rrnRaw, ((frame.size - 14) / 2).coerceAtLeast(0))
        parsed["rr_count"] = rrn
        val rrs = ArrayList<Int>()
        for (i in 0 until rrn) {
