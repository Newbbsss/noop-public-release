    ...
    for (r in parsed) {
        ...
        when (r.typeName) {
            "REALTIME_DATA" -> {
                ...
                if (ts != null) {
                    // Match ingestStandardHr / LiveState gates — never bank spike / placeholder values.
                    p.intOrNull("heart_rate")?.let { bpm ->
                        if (bpm in 30..220) out.hr.add(HrSample(ts, bpm))
                    }
                    // Drop RR rows when timestamp is absent (a ts-less RR row is unstorable).
                    p.intArrayOrNull("rr_intervals")?.let { rrs ->
                        for (rr in rrs) if (rr in 250..3000) out.rr.add(RrInterval(ts, rr))
                    }
                }
            }
