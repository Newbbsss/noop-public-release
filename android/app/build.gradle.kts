...
// Optional release signing. Credentials live in `keystore.properties` (git-ignored).
// Without it, release signing falls back to the debug key so assembleRelease stays installable.
// See docs/BUILD.md.
...
android {
    ...
    signingConfigs {
        // Gilbert fork device installs use fork-debug.keystore (SHA-256 4511d903...).
        // Prefer it for BOTH debug and release so store OTA matches the phone.
        getByName("debug") {
        ...
        create("forkDebug") {
            val forkDebugKeystore = rootProject.file("fork-debug.keystore")
            require(forkDebugKeystore.exists()) {
                "fork-debug.keystore missing at ${forkDebugKeystore.absolutePath}. " +
                    "MAIN/store builds must use it (cert 4511d903) or OTA will fail on the device."
            }
            storeFile = forkDebugKeystore
            storePassword = "android"
            keyAlias = "androiddebugkey"
    ...
    buildTypes {
        ...
        release {
            ...
            // Gilbert MAIN/store: always fork-debug when present (matches device / 8.6.72).
            // Optional keystore.properties only when explicitly requested via -PuseReleaseKeystore.
            signingConfig = when {
                rootProject.file("fork-debug.keystore").exists() &&
                    !project.hasProperty("useReleaseKeystore") ->
                    signingConfigs.getByName("forkDebug")
