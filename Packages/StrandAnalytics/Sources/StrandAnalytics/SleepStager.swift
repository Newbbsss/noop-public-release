...
public enum SleepStager {
    ...
    /// Soft-snap bed/wake to longest band-asleep stretch (Fable Sleep #27/#28). Slack ±90 min.
    public static let bandBoundSlackSec: Int = 90 * 60
    public static let bandBoundMinAsleepSec: Int = 90 * 60
    public static let bandBoundSampleGapSec: Int = 5 * 60
    ...
    /// Consecutive sleep epochs required to declare onset (parity with Android SleepStager = 5).
    public static let onsetPersistEpochs: Int = 5
    ...
    private static func detectSleepUncached(hr: [HRSample],
        ...
        for p in runs {
            ...
            let (bed, wake) = refineBoundsFromBandState(start: p.start, end: p.end, bandSleepState: bandSleepState)
            let stages = useSleepStagerV2
                ...
                : stageSession(start: bed, end: wake, grav: grav,
                               hr: hrS, rr: rrS, resp: respS)
            let eff = efficiency(start: bed, end: wake, stages: stages)
            let avgHrv = sessionAvgHRV(start: bed, end: wake, rr: rrS)
            let restingFinal = (bed != p.start || wake != p.end)
                ? sessionRestingHR(start: bed, end: wake, hr: hrS)
                : resting
            sessions.append(SleepSession(start: bed, end: wake, efficiency: eff,
                                         stages: stages, restingHR: restingFinal, avgHRV: avgHrv))
            traceSink?(GateTrace.runLine(index: runIndex, startTs: bed, endTs: wake,
                verdict: .kept, gate: "accepted",
                detail: "spanMin=\((wake - bed) / 60) eff=\(round2(eff)) restingHR=\(restingFinal ?? -1) daytime=\(isDaytime)"))
            // A run that does NOT continue the chain re-anchors it on this run's onset.
            if !continuesChain { chainFromOvernight = isOvernightOnset(bed, tzOffsetSeconds: tzOffsetSeconds) }
            chainPrevEnd = wake
        }
    ...
    public static func refineBoundsFromBandState(
        start: Int, end: Int,
        bandSleepState: [(ts: Int, state: Int)],
        slackSec: Int = bandBoundSlackSec,
        minAsleepSec: Int = bandBoundMinAsleepSec,
        gapSec: Int = bandBoundSampleGapSec
    ) -> (Int, Int) {
        guard !bandSleepState.isEmpty, end > start else { return (start, end) }
        let lo = start - slackSec
        let hi = end + slackSec
        let asleep = bandSleepState
            .filter { $0.state == bandStateAsleep && $0.ts >= lo && $0.ts <= hi }
            .map(\.ts)
            .sorted()
        guard asleep.count >= 3 else { return (start, end) }

        var bestA = asleep[0], bestB = asleep[0]
        var runA = asleep[0], runB = asleep[0]
        for i in 1..<asleep.count {
            let t = asleep[i]
            if t - runB <= gapSec {
                runB = t
            } else {
        ...
        if runB - runA > bestB - bestA { bestA = runA; bestB = runB }
        let span = bestB - bestA
        guard span >= minAsleepSec else { return (start, end) }
        guard bestB >= start, bestA <= end else { return (start, end) }
        guard abs(bestA - start) <= slackSec, abs(bestB - end) <= slackSec else { return (start, end) }
        let gravSpan = end - start
        guard span >= gravSpan / 2 else { return (start, end) }
        let newStart = min(max(bestA, lo), hi)
        let newEnd = min(max(bestB + 60, lo), hi)
        guard newEnd - newStart >= minAsleepSec else { return (start, end) }
