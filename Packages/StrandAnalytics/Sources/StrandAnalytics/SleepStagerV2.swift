...
public enum SleepStagerV2 {
    ...
    /// and slightly lowers deep prior so light wins weak-evidence epochs.
    static let baseLogPrior: [String: Double] = [
        "light": log(0.50), "deep": log(0.15), "rem": log(0.22), "awake": log(0.34)]

    /// Deep-eligibility HR-flatness percentile gate. Raised → 0.40 by DREAMT (n=100) (#348).
    public static let deepGateThresh = 0.40
    static let deepGateSlope = 5.0

    /// Motion thresholds relative to night quiescent jerk floor. DREAMT: move 38→75, gate 55→35, boost 2→4.
    static let jerkFloorMoveMult = 75.0
    static let jerkFloorGateMult = 35.0
    ...
    static func stageEpochs(_ feats: [Epoch]) -> [String] {
        ...
        for f in feats {
            ...
            var em: [String: Double] = [
                // Emission weights from DREAMT (#348) joint re-tune.
