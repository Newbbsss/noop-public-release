...
/// gyroscope @640/@840/@1040 (#493); v20 (2140 B) carries five repeated 422-byte optical blocks
/// (21-byte header + two 200-B channel slots + reserved). Header byte 0 is the shared sample count
/// (typically `[25,0,0,25,25]`); the two slots under one header are a paired detector/readout, not two
/// wavelengths (#546 / Whoop5RawOptical). Tests exercise decode on CRC-valid assembled frames — plus
/// one real captured v21 buffer for the gravity-shell identity check.
final class Whoop5HistoricalV2021Tests: XCTestCase {
    ...
    func testV20HeaderActiveAndEmptyBlocks() {
        ...
        let frame = makeFrame(total: 2140, version: 20) { f in
            ...
            putU32(&f, 11, idx)
            putU32(&f, 15, unix)
            // Block 0 active: header byte 0 = sample count 25; two paired i32 channels.
            f[0x1a] = 25
            for i in 0..<25 { putI32(&f, 0x2f + i * 4, Int32(100000 + i)) }   // ch b0_0
            for i in 0..<25 { putI32(&f, 0xf7 + i * 4, Int32(200000 - i)) }   // ch b0_1
            // Block 1 empty: sample count stays 0.
            f[0x1c0] = 0x00
            // Block 3 active (gated tail block): count 25 + both channel slots.
            f[0x50c] = 25
            for i in 0..<25 { putI32(&f, 0x521 + i * 4, Int32(140 + i)) }
            for i in 0..<25 { putI32(&f, 0x5e9 + i * 4, Int32(130 + i)) }
        }
        let p = parseFrame(frame, family: .whoop5)
        ...
        XCTAssertEqual(p.parsed["record_index"]?.intValue, Int(idx))
        XCTAssertEqual(p.parsed["unix"]?.intValue, Int(unix))
        XCTAssertEqual(p.parsed["sensor_block_count"]?.intValue, 5)
        ...
        XCTAssertNil(p.parsed["red"])
        XCTAssertNil(p.parsed["ir"])
    }
}
