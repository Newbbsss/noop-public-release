...
//
// 2026-07-19 (Fold MG live capture, auto-gather-20260718/203825, 181/181 frames): the SAME body also
// rides the LIVE REALTIME_RAW_DATA carrier (inner type 0x2B, char fd4b0005) — identical block offsets,
// counts [25,0,0,25,25], ts@15 monotonic 1 s steps, recordIndex@11 monotonic +1/frame. Channel-slot
// metadata byte 0 is a stable per-slot channel id (blk0: slots 03/04 at ~187k/~70k ADC, same-second
// waveforms correlating 0.95 at lag 0; blk3: 03/04 near-dark ~400 = ambient reference; blk4: 03 + 01).
// Distinct ids are structural evidence of distinct readout channels — still NOT wavelength names and
// never SpO2 %. The 244-byte live v20 frame is a separate short variant (block0 slot 0 only, 25 u32
// @47, count@26=25, ts@15, zero-padded tail) and stays rejected here.
...
public enum Whoop5RawOptical {
    ...
    /// Type byte: 0x2F = type-47 HISTORICAL_DATA offload; 0x2B = live REALTIME_RAW_DATA carrier — same
    /// v20 body proven on both (2026-07-19 live capture, see header doc). Accept either.
    public static func decode(_ frame: [UInt8]) -> Whoop5OpticalFrame? {
        guard frame.count == bufferLength,
              frame[0] == 0xAA,
              frame[8] == 0x2F || frame[8] == 0x2B,
