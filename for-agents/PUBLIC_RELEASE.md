...
- **Public** `versionName` is plain (e.g. `8.6.242`) â€” see `android/app/build.gradle.kts` in this tree.
...
Tools\sync_public_release.ps1 `
  -PublicVersionName 8.6.242 `
  -PublicVersionCode 512 `
...
git add -A
git commit -m "Public release 8.6.242"
git push origin main

# 4) Attach MAIN APK to a GitHub Release (tag v8.6.242)
gh release create "v8.6.242" .\NOOP-v8.6.242-main.apk `
  -R Newbbsss/noop-public-release `
  --title "NOOP 8.6.242 Public Release" `
...
- Prefer building MAIN from **this** scrubbed tree so `versionName` matches `8.6.242`.
