...
- **Public** `versionName` is plain (e.g. `8.6.168`) â€” see `android/app/build.gradle.kts` in this tree.
...
Tools\sync_public_release.ps1 `
  -PublicVersionName 8.6.168 `
  -PublicVersionCode 438 `
...
git add -A
git commit -m "Public release 8.6.168"
git push origin main

# 4) Attach MAIN APK to a GitHub Release (tag v8.6.168)
gh release create "v8.6.168" .\NOOP-v8.6.168-main.apk `
  -R Newbbsss/noop-public-release `
  --title "NOOP 8.6.168 Public Release" `
...
- Prefer building MAIN from **this** scrubbed tree so `versionName` matches `8.6.168`.
