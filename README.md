> **Public Release** ([https://github.com/Newbbsss/noop-public-release](https://github.com/Newbbsss/noop-public-release)) â€” Gilbertâ€™s scrubbed public channel of NOOP.
...
<p align="center">
  <a href="#download">â¬‡&nbsp;Download</a> Â·
  <a href="https://github.com/ryanbr/noop/wiki/FAQ">â“&nbsp;FAQ</a> Â·
  <a href="https://discord.com/invite/nHK9FHczu">ðŸ’¬&nbsp;Discord</a> Â·
  <a href="https://www.reddit.com/r/NOOPApp/">ðŸ‘½&nbsp;Reddit</a> Â·
...
|---|---|---|
| **macOS** | `NOOP.app` (see [Releases](https://github.com/ryanbr/noop/releases)) or Homebrew: `brew tap noopapp/noop && brew trust noopapp/noop && brew install --cask noop` | Apple Silicon + Intel. Drag to Applications. Not notarized â€” see **First launch on macOS** below. The one-time `brew trust noopapp/noop` is needed on Homebrew 6.0+ (harmless on older versions) â€” see [Homebrew docs](docs/HOMEBREW.md). |
| **Android** | `NOOP-full.apk` (see [Releases](https://github.com/ryanbr/noop/releases)) | The full app. `minSdk 26` (Android 8+). Sideload â€” enable "install unknown apps". Blocked by Play Protect? See **Installing on Android** below. |
| **iOS** | `NOOP-vX-ios.ipa` (see [Releases](https://github.com/ryanbr/noop/releases)) â€” sideload with AltStore/SideStore | Now a **direct download**. The `.ipa` is unsigned; **you** sign it on your iPhone with your own free Apple ID (no App Store, no developer account â€” NOOP stays anonymous). Re-signs every 7 days (AltStore automates it); Apple Health + Live Activity widgets may be limited under a free signing identity. See [docs/IOS.md](docs/IOS.md). Or build from source in Xcode. |
...
>   **Play Store â†’ your profile icon â†’ Play Protect â†’ âš™ Settings**, toggle **"Scan apps with Play
...
- **Name a strap reliably:** use **Devices â†’ Name in NOOP**. It is a local, restart-safe label for every device. The separate Bluetooth-name action is a WHOOP 4.0-only command and should be confirmed after reconnecting; NOOP does not offer it on 5/MG.
- **Plan sleep without fake precision:** set an earliest wake time and window under **Alarm**. After three recorded nights, NOOP turns your own sleep average into a bedtime cue. Early wake is an HR-based cue, not sleep-stage detection; the phone deadline remains the backup.
- **Charge with less guesswork:** the charging screen shows live battery and a clearly labeled time-to-full estimate. WHOOP does not expose an official charge rate or limit through the open Bluetooth path.
- **Help sport ML honestly:** save or correct workouts in a debug build. See [Sport ML Collection](docs/SPORT_ML_COLLECTION.md); NOOP is collecting user-confirmed session labels before it attempts sport classification.
- **Retrieve 5/MG data safely:** use normal Sync first, then raw capture or R22 only as intentional, opt-in debugging. See [WHOOP 5/MG deep data](docs/WHOOP5_DEEP_DATA.md).
...
| **WHOOP 5.0 / MG** | ðŸ§ª **Live heart rate works** (confirmed on real hardware). Pick "WHOOP 5.0 / MG" before connecting â€” and see the pairing note below, because you can't just scan for it. Deeper 5/MG metrics (recovery, strain, sleep) are still being mapped; there's an opt-in **Settings â†’ Experimental** toggle for 5/MG owners who want to help document the protocol. |
...
# Select the "Strand" scheme â†’ Run (âŒ˜R). The built app is named NOOP.
```
...
NOOP is public and built to be hard to erase. **Clone it freely** â€” `git clone https://github.com/ryanbr/noop.git` â€” and you're welcome to **mirror or fork it** to Codeberg, GitLab or your own server. More copies make the project more resilient, which is the whole point after being deplatformed.

Two simple asks:

- **Keep it non-commercial** and keep the [`LICENSE`](LICENSE) + `Copyright 2026 NoopApp` notice intact (PolyForm Noncommercial â€” mirror and use freely, just don't sell it or ship it in a paid product).
...
If NOOP's useful to you, a â­ genuinely helps it reach more WHOOP users â€” and it's the single best free way to support the project.
